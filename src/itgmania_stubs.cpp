...
// ---------------------------------------------------------------------------
// Minimal RageFile backed by std::ifstream/std::ofstream.
...
void RageFileManager::GetDirListing(const RString& path, std::vector<RString>& out, bool onlyDirs, bool returnPathToo) {
    ...
    for (const auto& entry : directory_iterator(path.c_str(), ec)) {
        ...
        RString name = returnPathToo ? entry.path().string().c_str() : entry.path().filename().string().c_str();
        out.push_back(name);
    }
}
void RageFileManager::GetDirListingWithMultipleExtensions(const RString& path,
    std::vector<RString> const& exts, std::vector<RString>& out,
    bool onlyDirs, bool returnPathToo) {
    using std::filesystem::directory_iterator;
    out.clear();
    std::error_code ec;
    for (const auto& entry : directory_iterator(path.c_str(), ec)) {
